...
TEST_JSON = "C:\\PycharmProjects\\VR89_Test_Suite\\tests\\offset_remedy\\test_data_jarek.json"
...
def test_jarek(run_scats, test_data):
    ...
    common_methods.set_data_available(site, INT_17_DETECTORS)
    if lane_data is not None:
        set_platoon_values_for_si(test_data[SI_NUM], lane_data)
    common_methods.move_the_clock(subsystem, test_data[CG_STEP], test_data[ROTATION_CYCLES])
    get_offset_remedy_results(SLOT1, test_data[EXPECTED_OFFSETS])


def set_up():
    common_methods.set_x_key(False, False, False)
    dim_si = ScatsPythonPlugin.GetMaxSICount()
    for si in range(1, dim_si):
        ScatsPythonPlugin.ClearSI(si)
    common_methods.move_the_clock(SS1, CG4, NO_SPIN)
    subsystems = [SS1, SS2, SS3]
    common_methods.set_subsystem_cycle_time(subsystems, CYCLE_TIME)
    common_methods.set_subsystem_active_cycle_time(subsystems, ACTIVE_CYCLE_TIME)
    # ScatsPythonPlugin.AttemptSSConnect(SS2)
    # ScatsPythonPlugin.AttemptSSConnect(SS3)
...
def get_offset_remedy_results(from_slot, expected_results):
    ...
    for slot, expected_offset in expected_results:
        ...
        assert check_offset_temp(from_slot_time, to_slot_time, expected_offset, from_active_cycle_length)
